...
// Secret unlock for the #220 easter egg: mash the logo — "força bruta"
// mirrors the joke on that page (Emy waiting for a hammer to fix things).
// Not documented anywhere in the UI on purpose.
const LOGO_MASH_THRESHOLD = 8
const LOGO_MASH_WINDOW_MS = 2000
...
const NavBar = ({ ambienceEnabled, onToggleAmbience, onNavigate }: NavBarProps) => {
  ...
  const handleLogoClick = useCallback(() => {
    const now = Date.now()
    const recentClicks = logoClickTimestampsRef.current.filter((t) => now - t < LOGO_MASH_WINDOW_MS)
    recentClicks.push(now)

    if (recentClicks.length >= LOGO_MASH_THRESHOLD) {
      logoClickTimestampsRef.current = []
      onNavigate('#220')
      return
    }

    logoClickTimestampsRef.current = recentClicks
    onNavigate('#hero')
  }, [onNavigate])
  ...
  return (
    <header className={`mateka-navbar ${scrolled ? 'is-scrolled' : ''}`}>
      <MatekaLogo onClick={handleLogoClick} />
